...
**Real withdraw (added 2026-09-13).** The [live demo](https://stellar-zkstream.vercel.app/) used to be able to create a stream but never claim from one. It now has a "Withdraw Demo Stream" panel that submits a real Groth16 nullifier proof against a real pre-created demo stream (#1) with a real 5,000,000-stroop escrow — see `frontend/src/soroban.ts` for how that proof was generated with the existing compiled circuit (no trusted-setup repeat needed) and confirmed end-to-end via a real, separate throwaway-secret CLI withdrawal first. Unlike `create_stream`'s fixed proof, this one is necessarily tied to one specific stream: the nullifier circuit's public inputs bind to `stream_id`, so the proof can't be reused for an arbitrary stream the way the range proof can be reused for an arbitrary recipient. It also only works once, ever, and only for that stream's actual recipient — any other connected wallet gets a real, honest on-chain revert, the same pattern `stellar-zkident`'s faucet/credential demos already use for write actions gated to one fixed identity.
